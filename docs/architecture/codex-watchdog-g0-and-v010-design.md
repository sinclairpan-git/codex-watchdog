# Feishu × Codex Watchdog G0 总设计与 V010 冻结设计

- 状态：Frozen for spec/plan/tasks
- 日期：2026-04-05
- 对应工作项：`010-runtime-integration-spine`
- 对应主题：`Feishu Integration Spine`
- 目标契约版本：`watchdog-session-spine/v1alpha1`
- 目标 schema 版本：`2026-04-05.010`
- 关联真值：
  - `codex-watchdog-prd.md`
  - `specs/001-codex-watchdog-foundation/spec.md`
...
1. `008`：Codex runtime service 到本机 Codex live session 的真实控制闭环。
2. `009`：Codex runtime service 与 Watchdog 侧的基础 SSE 任务事件流。

但对 Feishu 来说，当前仍缺一层真正可长期演进的**稳定集成脊柱**。现有能力更多仍是：

- Codex runtime service 的原始任务对象与审批对象；
- Watchdog 的 `progress / evaluate / approvals / recover / events` 原子接口；
- 面向当前实现的字段形状，而不是面向 Feishu 的中立稳定契约。

`010-runtime-integration-spine` 的唯一目标固定为：

> 建立 Feishu 可稳定消费的会话监管骨架，完成 `intent -> session projection / watchdog action -> reply model` 的最小可用闭环，而非实现完整实时监管与自动恢复体系。
...
1. **010 不是 Watchdog 内的一个 `feishu` 子服务，而是“中立 contract 包 + L2 投影层 + L3 独立 adapter spine”。**
2. **Feishu 只消费 Watchdog 的 stable contract，不直接消费 Codex runtime service raw task schema，也不直连 SSE / recover / approvals raw 接口。**
...
```mermaid
flowchart LR
    U["Feishu Query / Action"] --> L3["L3 Feishu Adapter Spine"]
...
- 不使用 `feishu` 命名；
- 不携带飞书或渠道 runtime 语义；
- 只冻结 Feishu 与 Watchdog 之间长期稳定的对象、枚举、版本字段和动作模型。
...
- `evaluate_stuck(...)` 的结果
- Codex runtime service 可达性与线程映射状态
...
它们都**不是** Feishu 的 canonical stable contract。
...
1. Codex runtime service `task` envelope。
2. Codex runtime service approvals 列表与审批对象。
...
010 设计的 stable API 只面向 Feishu 集成骨架，不替代现有 raw API。
...
4. 幂等收据存储属于 Watchdog 自身职责，不得下沉到 Feishu adapter。
...
- 仍可复用现有 Codex runtime service decision 路径；
- 但对 Feishu 暴露的必须是 `WatchdogActionResult` / `ReplyModel(reply_code=approval_result)`；
...
1. 把 stable contract 放回 `src/watchdog/services/feishu/*`。
...
1. Feishu 面向的是中立 contract 包，而不是 Watchdog 内部 `feishu` 子服务。
