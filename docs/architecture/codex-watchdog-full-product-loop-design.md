# Feishu × Codex Watchdog Full Product Loop 总设计
...
1. `Codex runtime service(A)` 对本机 Codex 会话的真实读取与最小控制闭环；
2. `Watchdog(B)` 面向 Feishu 的 stable session spine、stable facts、stable explanations、stable action/read surface；
3. Feishu 可直接复用的 HTTP 路由模板与接入参数收敛。

但当前形态仍然是 **stable spine / query+action MVP**，还不是你要求的最终产品闭环。明显缺口包括：

- 任务阻塞、待办、暂停、等待建议时，还没有常驻的主动提醒链路；
- 自动决策、人工升级、审批闭环与事后通知，还没有被收成可执行策略；
- Feishu 侧目前只有模板化接法，没有稳定 webhook / response contract 与 reference runtime；
- 公网入口、可靠投递、审计、回放、运维与生产化仍停留在“能跑”，不是“可长期运营”。

这份总设计的唯一目标固定为：

> 把当前 `Codex runtime service(A) + Watchdog(B) + Feishu + Feishu` 从 stable spine 形态升级成“默认自治、例外升级、主动通知、可审计、可生产化”的完整产品闭环。

## 2. 系统边界

本轮边界冻结为：

- **Feishu**：只负责把消息展示给用户，并把用户输入回传给 Feishu。
- **Feishu**：只负责宿主适配。
...
- **Codex runtime service(A)**：只负责读取 Codex 事实与执行 continue / steer / approval callback / recovery 之类的 canonical 动作，不承担提醒编排与宿主回调。

```mermaid
flowchart LR
    FEI["Feishu\nDisplay + User Input"] --> OC["Feishu\nHost / Channel Adapter"]
    OC --> WD["Watchdog(B)\nSession Spine + Policy + Delivery"]
    WD --> ACA["Codex runtime service(A)\nFacts + Execution"]
    ACA --> CODEX["Codex\nDesktop / app-server / local state"]
```

### 2.1 runtime 侧边界

`Codex runtime service(A)` 只负责：

- 原始会话与审批事实采集；
- 本机 Codex thread / turn / approval 相关 canonical 动作执行；
- 为 B 提供稳定、可鉴权的事实与动作接口。

`Codex runtime service(A)` 不负责：
...
- 对 Feishu 的 webhook 与 response API。
...
- Feishu 必须被视为**弱记忆、弱纪律、可随时重启**的宿主 I/O 层；
- 任何关键状态、顺序语义、待处理事项、补发责任与恢复责任，都必须持久化在 `Watchdog(B)`，不得寄托于 Feishu 记忆。
...
1. `Projection Worker`
   - 持续从 `Codex runtime service` 拉取或接收事实变化；
   - 更新 canonical session spine；
   - 生成新的 `fact_snapshot_version`。
2. `Resident Orchestrator`
   - 固定按 `session spine refresh -> policy evaluate -> auto recovery / approval materialize -> enqueue delivery -> call Feishu` 的链路常驻推进；
   - 对新的事实快照做策略判定；
   - 在低风险时执行 canonical action，在高风险时物化 approval；
   - 对普通进展变化生成节流后的 `progress_summary` 主动推送。
3. `Delivery Worker`
   - 把 envelope 主动回调给 Feishu；
   - 处理重试、去重、receipt、失败升级。

这一节同时冻结一条总原则：

- `Watchdog(B)` 必须在后台常驻推进流程；Feishu 只承担展示、回传与外部 I/O，不承担“记住还有什么待办”的职责。
...
Watchdog 主动调用 Feishu 的固定入口冻结为：

- `POST /feishu/v1/watchdog/envelopes`

请求头：

- `Authorization: Bearer <watchdog_to_feishu_token>`
- `X-Watchdog-Delivery-Id: <envelope_id>`
- `X-Watchdog-Timestamp: <ts>`
- `X-Watchdog-Signature: <hmac>`

Feishu 成功接收后，返回体至少包含：
...
Feishu 回传 Watchdog 的固定入口冻结为：

- `POST /api/v1/watchdog/feishu/responses`
...
- Feishu 只负责接收 envelope、渲染、接收用户输入、回传结构化 response。
- Feishu 不得重算决策、不做风险分类、不维护第二套审批状态机或 session spine。
- Feishu 即使丢失上下文、重启或“失忆”，系统真值也不能丢；`Watchdog(B)` 必须能依靠持久化 records 继续推进、补发或恢复待处理事项。

### 5.7 失败处理基线

完整产品闭环对“自动处理到哪一步”为止，也必须冻结：

- `context_critical / context window` 类问题，优先由 `Watchdog(B)` 通过 canonical recovery 链路自动处理；允许在同一项目内走 `resume_or_new_thread` 续跑，而不是把恢复责任交给 Feishu。
...
### 6.5 WI-5：Feishu Webhook / Response API / Reference Runtime

- 目标：把 Feishu 接入从口头说明收成稳定契约与宿主模板。
- 范围：
  - `POST /feishu/v1/watchdog/envelopes`
  - `POST /api/v1/watchdog/feishu/responses`
  - webhook 鉴权与签名
  ...
  - 最小 Feishu reference runtime / template
- 非目标：
  - 不做业务策略
  - 不做第二套状态机
  - 不做第二个 Watchdog 内核
...
flowchart TD
    ...
    WI4 --> WI5["WI-5\nFeishu Webhook + Response API + Reference Runtime"]
