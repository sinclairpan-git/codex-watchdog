...
def test_local_codex_client_rejects_case_variant_home_directory_as_project_id(
    tmp_path: Path,
) -> None:
    codex_home = tmp_path / ".codex"
    home = Path.home()
    case_variant_home = Path(str(home).swapcase())
    rollout = codex_home / "sessions/2026/04/05/rollout-case-home-cwd.jsonl"
    _seed_codex_home(
        codex_home,
        threads=[
            {
                "thread_id": "thr_case_home_cwd",
                "cwd": str(case_variant_home),
                "task_title": "Ignore home cwd case variant",
                "updated_at": 200,
                "rollout_path": rollout,
                "events": [
                    {
                        "timestamp": "2026-04-05T00:00:00Z",
                        "type": "session_meta",
                        "payload": {"id": "thr_case_home_cwd", "cwd": str(case_variant_home)},
                    }
                ],
            }
        ],
    ...
    client = LocalCodexClient(codex_home=codex_home)
    session = client.describe_thread("thr_case_home_cwd")

    assert session["project_id"] == "unknown-project"
    assert client.list_threads() == []


def test_local_codex_client_rejects_codex_internal_directory_as_project_id(
    tmp_path: Path,
) -> None:
    codex_home = tmp_path / ".codex"
    rollout = codex_home / "sessions/2026/04/05/rollout-codex-cwd.jsonl"
    _seed_codex_home(
        codex_home,
        active_workspaces=[str(codex_home)],
        threads=[
            {
                "thread_id": "thr_codex_cwd",
                "cwd": str(codex_home),
                "task_title": "Ignore Codex state dir",
                "updated_at": 200,
                "rollout_path": rollout,
                "events": [
                    {
                        "timestamp": "2026-04-05T00:00:00Z",
                        "type": "session_meta",
                        "payload": {"id": "thr_codex_cwd", "cwd": str(codex_home)},
                    }
                ],
            }
        ],
