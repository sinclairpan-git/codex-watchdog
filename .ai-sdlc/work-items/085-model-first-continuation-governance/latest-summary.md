...
- `WI-085` 已完成；`ContinuationPacket` 现正式成为 recovery / handoff / resume 的 source of truth，runtime-side handoff markdown 与 recovery-resume prompt 都退化为 packet 的 deterministic render，不再把渲染文本回流成 authoritative continuation 输入。
...
- `T857` 已完成：`ContinuationPacket` hash / rendered-from-packet hash / source_packet_id 已在 `SessionService`、operator control-plane、Feishu adapter、recovery truth write 面贯通，`handoff_summary` 文本不再承担 route/dedupe/model truth 角色。
