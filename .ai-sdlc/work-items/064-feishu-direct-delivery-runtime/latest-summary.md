...
- `WI-064` 目标是补齐 Feishu direct delivery runtime，让 outbound 不再只能走 Feishu compatibility webhook。
- 当前已完成：
  - `FeishuAppDeliveryClient` 已支持 tenant token 获取、文本消息发送与 message id receipt；
  - `create_app()` 已按 `delivery_transport` 在 `feishu-app` 与 `feishu` 之间切换；
  - unknown transport 现在 fail-closed，不再静默回退到 Feishu；
...
- `uv run pytest -q tests/test_watchdog_feishu_control.py tests/test_feishu_contracts.py tests/test_watchdog_ops.py` -> `36 passed in 1.07s`
