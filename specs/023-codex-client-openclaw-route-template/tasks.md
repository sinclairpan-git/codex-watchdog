# 任务分解：023-codex-client-openclaw-route-template

> 对应规格：`specs/023-codex-client-openclaw-route-template/spec.md`

## Task 23.1 冻结接入边界与模板输入项

- **任务编号**：T231
- **状态**：已完成（2026-04-06 手工补齐）
- **目标**：把真实 Codex 客户端、OpenClaw 路由模板与外部前提清单收敛到 formal docs。
- **涉及文件**：
  - `specs/023-codex-client-openclaw-route-template/spec.md`
  - `specs/023-codex-client-openclaw-route-template/plan.md`
  - `specs/023-codex-client-openclaw-route-template/tasks.md`
  - `specs/023-codex-client-openclaw-route-template/task-execution-log.md`
  - `docs/getting-started.zh-CN.md`
- **完成标准**：
  1. 023 scope 明确限定为“真实 Codex 客户端接缝 + OpenClaw 路由模板”；
  2. 已列出网络地址、Token 策略、Codex/OpenClaw 接口样例等外部前提；
  3. 已明确说明不覆盖飞书/OpenClaw 运行时代码。
...
## Task 23.3 接 A-Control-Agent API 与审计语义
...
## Task 23.4 交付 OpenClaw 路由模板与接入说明

- **任务编号**：T234
- **状态**：已完成（2026-04-06）
- **目标**：提供可直接给 OpenClaw 集成方复用的查询/控制/审批路由模板与环境变量说明。
- **涉及文件**：
...
- **完成标准**：
  ...
  3. 示例与文档明确说明本仓库不包含飞书/OpenClaw runtime，但已提供可复用模板。
- **验证**：`uv run pytest -q tests/integration/test_openclaw_integration_spine.py`

## 整体验收

- 当前仓库已经从“023 前仅有 stable API 与最小 HTTP 示例”推进到“有真实 Codex 客户端接缝、有 OpenClaw 路由模板、可直接指导现场集成”。
- 023 不改写 010-022 已冻结的 stable contract，只补真实接入层与模板化配置。
- `.ai-sdlc` 顶层 state、checkpoint 与 formal docs 已统一指向 `023-codex-client-openclaw-route-template`。
