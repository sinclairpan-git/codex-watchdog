# 执行日志：049-feishu-and-openclaw-entrypoint-closure
...
- 当前收紧后的 049 边界：
  - 只做 Feishu 主入口、OpenClaw compatibility 入口与自然语言到 canonical route 的收口；
...
- 当前 canonical branch 已切到 `codex/049-feishu-and-openclaw-entrypoint-closure`。
- 当前下一执行入口切换为 `T492`：先写失败测试锁定 Feishu / OpenClaw / natural-language entry contract，再进入实现。

### Phase 2：red tests freeze Feishu / OpenClaw entry contract
...
- 先补两条最小红测，锁定入口层必须消费正式 canonical route：
  ...
  - `tests/test_watchdog_openclaw_adapter.py::test_adapter_routes_natural_language_progress_message_to_canonical_reply`
- 首轮红测结果：
  - `uv run pytest -q tests/test_watchdog_feishu_control.py tests/test_watchdog_openclaw_adapter.py`
  - 结果：`2 failed, 21 passed`
  - 失败原因固定为：
    - Feishu `command_request` 尚未接到 canonical query/action route；
    - `OpenClawAdapter` 尚不支持 natural-language `handle_message(...)` contract。
- 第二轮继续补充控制动作与线程路由红测，冻结 049 的剩余 contract：
  - Feishu `暂停` 命令必须返回 canonical `pause_session` 回执，并把 receipt 落到 `ActionReceiptStore`；
  - Feishu / OpenClaw 必须支持 `native_thread_id -> project_id` 路由；
  - OpenClaw compatibility 入口对外仍暴露自然语言 `get_session` 语义，不泄漏内部 `get_session_by_native_thread` 路由细节。
- 扩展红测结果：
  - `uv run pytest -q tests/test_watchdog_feishu_control.py tests/test_watchdog_openclaw_adapter.py tests/integration/test_openclaw_integration_spine.py`
...
- 已把自然语言入口解析收口到共享 helper：
  ...
  - 不再让 Feishu 和 OpenClaw 各自维护一套 project/native-thread 解析分支。
...
### Phase 4：OpenClaw compatibility route 收口

- `src/watchdog/services/adapters/openclaw/adapter.py` 已新增/收口：
...
- `src/watchdog/services/adapters/openclaw/intents.py` 已作为共享 intent catalog 使用，Feishu 与 OpenClaw 均复用同一套短指令映射。
- 集成验证结果：
  - `uv run pytest -q tests/test_watchdog_feishu_control.py tests/test_watchdog_openclaw_adapter.py tests/integration/test_openclaw_integration_spine.py`
  - 结果：`47 passed in 0.79s`
- 当前 `WI-049` 的正式入口闭环已具备以下证据：
  - Feishu primary entry 可直接查询/控制 canonical route；
  - OpenClaw compatibility entry 不再维护独立自然语言语义；
