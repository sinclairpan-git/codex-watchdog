...
# Feishu And OpenClaw Entrypoint Closure — 功能规格说明

## 概述

`049-feishu-and-openclaw-entrypoint-closure` 是 `WI-048` 完成 runtime semantics baseline 之后的正式后续 work item。它只消费 `matrix-owner-ledger.yaml` 中 owner=`WI-049` 的 92 条矩阵行，目标是把“已有 canonical route 与 runtime semantics，但仍缺正式入口、渠道接线、自然语言语义映射与主链路验证”的缺口收敛成稳定的 entry surface。
...
- **OpenClaw compatibility entrypoint**：让 OpenClaw 只作为兼容入口消费 048 冻结的 canonical read/action surface，不再自行发明语义；
- **natural-language mapping**：把用户在 Feishu / OpenClaw 中输入的自然语言短指令稳定映射到 canonical query / action route，而不是继续停留在示例或 operator 约定。
...
- `WI-036` 已冻结 “Feishu primary control plane，OpenClaw compatibility-only” 的总体迁移边界。
...
- **自然语言语义映射**
  ...
  - `matrix-row-0699` OpenClaw 侧把飞书消息转成 Watchdog 接口调用并做项目路由
- **OpenClaw compatibility-only 入口**
  - `matrix-row-0699` OpenClaw 侧项目路由与调用映射
  - `matrix-row-1329` 飞书成为唯一主控制面，OpenClaw 只保留兼容读写
...
补充边界说明：`matrix-row-0013 / 0015 / 0017` 这类“无实现、无验证、无入口”的条目仍然属于 `WI-048` 的语义实现范围。049 只负责把 048 已交付的 semantic effect、reason code 与 canonical action surface 暴露给 Feishu / OpenClaw / natural-language 入口消费。
...
- **FR-4904**：049 必须让 OpenClaw compatibility entrypoint 复用共享的 intent catalog / natural-language mapping / project routing，而不是继续在 adapter 或 examples 中散落 ad hoc 逻辑。
- **FR-4905**：049 必须支持稳定项目路由，至少允许通过 `project_id`、`native_thread_id` 或与当前 session 绑定的 entry context 解析到 canonical target session。
- **FR-4906**：049 必须保持 Feishu 为 primary control plane，OpenClaw 只保留 compatibility-only 读写入口；任何新增主链路能力都不能先落在 OpenClaw 独占 surface 上。
- **FR-4907**：049 必须为 Feishu / OpenClaw 入口加上受控入口纪律：继续复用 token 鉴权、canonical approval / receipt / audit / session event 写屏障，不允许入口层绕过审计。
...
1. reviewed formal artifact set 固定为以下 repo-root 相对路径，且顺序不得改变：
   ...
   - `specs/049-feishu-and-openclaw-entrypoint-closure/task-execution-log.md`
...
### 用户故事 2：OpenClaw 仍需要作为兼容层消费统一入口语义

作为维护者，我需要 OpenClaw 继续可用，但只以 compatibility-only 身份消费共享 intent catalog、project routing 和 canonical route，这样不会再次把 OpenClaw 变回主链路。

场景 1：OpenClaw 收到“继续 / 恢复 / 总结 / 审批列表”等短指令时，走与 Feishu 相同的 canonical intent 映射。

场景 2：OpenClaw 兼容入口的项目路由、native thread 解析与 action receipt 查询使用统一 contract，而不是散落在 examples 或单测替身里。

### 用户故事 3：入口层不能偷偷改变 048 已冻结的真相

作为系统维护者，我需要 Feishu / OpenClaw / natural-language 入口只做路由和消费，不重新发明任务状态、审批真相或 Memory precedence，否则 048 的 runtime baseline 会再次漂移。
