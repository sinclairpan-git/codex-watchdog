# 任务分解：049-feishu-and-openclaw-entrypoint-closure

> 对应规格：`specs/049-feishu-and-openclaw-entrypoint-closure/spec.md`
>
> 对应计划：`specs/049-feishu-and-openclaw-entrypoint-closure/plan.md`
...
- **验收标准**：
  ...
  2. 已明确 049 只做 Feishu / OpenClaw / natural-language 入口闭环，不重新定义 048 runtime semantics；
  3. 已把 checkpoint、branch、`next_work_item_seq` 与 docs baseline 收口到 049；
  4. 已明确 Feishu primary / OpenClaw compatibility-only 边界。
- **验证**：
...
- **任务编号**：T492
- **状态**：已完成
- **目标**：先用失败测试冻结 Feishu 主入口、OpenClaw compatibility 入口与自然语言映射 contract。
- **文件**：
...
- **验收标准**：
  ...
  3. 覆盖 OpenClaw compatibility 入口复用共享 mapping / routing；
...
## Task 49.4 实现 OpenClaw compatibility entry route 与 project routing 收口

- **任务编号**：T494
- **状态**：已完成
- **目标**：让 OpenClaw compatibility surface 复用共享 intent mapping / routing / canonical route。
- **文件**：
  - `src/watchdog/services/adapters/openclaw/adapter.py`
...
- **可并行**：否
- **验收标准**：
  1. OpenClaw 兼容入口不再维护独立语义；
  2. `project_id` / `native_thread_id` / entry context 可稳定路由到 canonical target；
  3. OpenClaw 继续保持 compatibility-only，不承接新主链路语义。
- **验证**：
  - `uv run pytest -q tests/test_watchdog_openclaw_adapter.py tests/integration/test_openclaw_integration_spine.py`
