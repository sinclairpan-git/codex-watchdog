...
在 `010-016` 完成 stable session spine、events、recovery、receipt、supervision、explanation 与 approval inbox 之后，Watchdog 对 OpenClaw 的 stable read surface 还剩一个明显缺口：
...
- 但“先枚举当前有哪些受监管会话，再决定查看哪个 session”的能力，仍只有 A-Control-Agent 的 raw `/api/v1/tasks`。

这意味着上层如果还不知道 `project_id`，就必须回退到 raw task schema，而不是继续消费 Watchdog 的稳定 session contract。

017 的唯一目标固定为：

> 建立 stable session directory read surface，让 OpenClaw 与其他调用方能够在未知 `project_id` 的情况下读取跨项目 `SessionProjection[]`，并继续通过同一套 stable contract 跳转到单会话读取、审批、解释或动作面。

## 功能需求

- **FR-1701**：017 必须补一个独立 stable session directory read surface；不得把 stable 语义直接塞回 A-Control-Agent 的 raw `/api/v1/tasks`。
...
- **FR-1705**：每个 `SessionProjection` 必须继续保留：
  ...
  - `available_intents`
- **FR-1706**：stable session directory 必须基于 A-Control-Agent `list_tasks()` 与 pending approvals 聚合构建，保证 `pending_approval_count` 和 `available_intents` 与单 session 读取口径一致。
- **FR-1707**：017 不新增新的 write action，也不改变 `POST /api/v1/watchdog/actions` 及其 alias 语义。
- **FR-1708**：OpenClaw adapter 必须新增 `list_sessions` intent，并返回与 HTTP stable route 同源的 `ReplyModel`；不得在 adapter 层自己拼 raw task list 文案。
...
- **FR-1711**：017 必须补齐 contract 测试、stable directory API 测试、adapter 测试、最小 integration 测试，并显式补一条 legacy `/api/v1/tasks` 非回归，确认 stable directory 不替换原始 A 侧任务列表。
- **FR-1712**：README、getting-started 与 OpenAPI 必须把 session directory 纳入 stable read surface，并明确：
  ...
  - `/api/v1/tasks`：A-Control-Agent raw task list，继续存在但不承担 stable contract 角色

### 用户故事 1：OpenClaw 在未知 project_id 时先拿目录

OpenClaw 需要先知道当前有哪些正在监管中的 session，再决定是否读取单 session、查看 explanation 或发起动作。
...
OpenClaw adapter `list_sessions` 与 HTTP stable route 应该读取同一份共享 builder，而不是各自维护一份 session list 映射。
...
- 不替换或删除 A-Control-Agent raw `/api/v1/tasks`。
