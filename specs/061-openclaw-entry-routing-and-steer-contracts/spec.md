# Spec：061-openclaw-entry-routing-and-steer-contracts

## 背景

`WI-060` 完成后，当前脏工作树中还剩下一条没有 formalize 的独立入口主题：

1. `OpenClawAdapter.handle_message(...)` 允许直接消费自然语言 message
...
`WI-061` 因此只承担 OpenClaw entry routing and steer contracts 的 formal closeout。
...
1. reviewed formal artifact set 固定为以下 repo-root 相对路径，且顺序不得改变：
   - `specs/061-openclaw-entry-routing-and-steer-contracts/spec.md`
   - `specs/061-openclaw-entry-routing-and-steer-contracts/plan.md`
   - `specs/061-openclaw-entry-routing-and-steer-contracts/tasks.md`
   - `specs/061-openclaw-entry-routing-and-steer-contracts/task-execution-log.md`
   - `.ai-sdlc/work-items/061-openclaw-entry-routing-and-steer-contracts/execution-plan.yaml`
   - `.ai-sdlc/work-items/061-openclaw-entry-routing-and-steer-contracts/latest-summary.md`
