...
- A-Control-Agent 已经提供 `GET /api/v1/tasks/{project_id}/workspace-activity`；
- `014` 的 supervision evaluation 已经消费同源 repo activity 信号；
- 但 OpenClaw 与其他上层调用方仍然没有稳定的 workspace-activity read surface，只能继续依赖 raw A payload。

019 的唯一目标固定为：

> 建立 stable workspace-activity read surface，让 OpenClaw 与其他调用方能够通过 Watchdog stable contract 获取版本化工作区活动摘要，而不是直接绑定 A-Control-Agent raw `/workspace-activity` 响应。
...
- **FR-1904**：`WorkspaceActivityView` 至少包含以下稳定字段：
  ...
  - `latest_mtime_iso`
  - `recent_change_count`
- **FR-1905**：019 必须在 Watchdog L2 内通过共享 builder 把 session 元信息与 A-Control-Agent `/workspace-activity` 原始摘要投影为 `WorkspaceActivityView`；HTTP route 与 adapter 不得直接拼 raw payload。
- **FR-1906**：OpenClaw adapter 必须新增 `get_workspace_activity` intent，并通过 stable builder 返回与 HTTP route 同源的 `ReplyModel`。
...
- **FR-1911**：README、getting-started 与 OpenAPI 必须明确写清：
  ...
  - `/api/v1/tasks/{project_id}/workspace-activity`：A-Control-Agent raw route，继续存在但不承担 stable contract 角色
- **FR-1912**：019 必须补齐契约测试、API 测试、adapter 测试、最小 integration 测试，以及 raw `/workspace-activity` 非回归测试。

### 用户故事 1：OpenClaw 可以稳定读取当前工作区活动摘要

OpenClaw 需要判断项目最近是否真的有文件活动，但不应该自己碰 raw A payload。
...
同一个 `project_id + recent_minutes` 在 HTTP stable route 与 OpenClaw adapter 上应得到同源结果。
...
已有脚本和历史测试可能仍依赖 A-Control-Agent raw route。
