...
# Feishu Integration Spine — 功能规格说明

## 概述

在 `008-codex-live-control-plane` 与 `009-task-events-stream` 已完成的基础上，`010-runtime-integration-spine` 不再扩张底层能力，而是补齐 **Feishu 可稳定消费的会话监管骨架**：通过 **中立 contract 包 + L2 稳定投影层 + L3 独立 adapter spine**，完成 `intent -> session projection / watchdog action -> reply model` 的最小可用闭环。
...
- **FR-1001**：010 必须先定义中立 contract 包，稳定对象、枚举与版本语义不得落在 `feishu` 命名空间下，也不得依附于渠道 runtime 语义。
...
- **FR-1004**：Watchdog 必须提供 L2 稳定投影层，把 raw task / approval / stuck signal / control-link state 归并为 `SessionProjection`、`TaskProgressView` 与 `FactRecord[]`，Feishu 不得直接依赖 Codex runtime service 原始任务对象或其字段形状。
...
- **FR-1014**：Feishu adapter spine 必须作为 L3 独立层存在，只消费中立 contract 与 stable API / service；不得直连 Codex runtime service、不得直连 SSE、不得混入飞书或其他渠道运行时代码。
...
### 用户故事 1：Feishu 稳定读取会话与进展

Feishu 需要获取项目会话的稳定身份、进展与解释，但不希望耦合到底层 task schema。
...
场景 3：即便底层 `TaskStore` 字段后续扩展，Feishu 仍只依赖稳定枚举、受控字段集与 `reply_code`。

### 用户故事 2：Feishu 解释“为什么卡住 / 为什么被阻塞”

Feishu 需要稳定回答“为什么卡住了”与“当前阻塞点是什么”，且答案不能依赖渠道端临时拼文案。
...
### 用户故事 3：Feishu 发起一次最小 continue 动作

Feishu 需要向 Watchdog 发出一次“继续推进”的最小稳定动作。
...
### 用户故事 4：Feishu 请求恢复建议但不执行恢复

Feishu 需要询问“当前是否适合恢复”，但 010 不应真的执行恢复动作。
...
### 用户故事 5：Feishu 稳定处理审批

Feishu 需要展示待审批项并允许人工通过或拒绝，且动作结果可重试。
...
- 不实现飞书或其他渠道运行时代码。
- 不让 Feishu 直连 Codex runtime service，也不让 stable API 原样透出 raw envelope。
