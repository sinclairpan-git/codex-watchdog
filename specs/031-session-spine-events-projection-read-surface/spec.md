...
- **FR-3107**：031 必须保持 010-024 既有 stable route contract 兼容；Feishu 模板、session query API 与 approval inbox API 的返回形状不得因底层投影来源切换而破坏调用方。
...
operator 与 Feishu 模板需要继续通过 `get_session / get_progress / why_stuck / explain_blocker / facts` 读取稳定视图，但底层真相不能再停留在 024 的 snapshot store。
...
- 不在 031 中重写 Feishu 模板、审批响应 API 或 delivery worker 的外部协议。
