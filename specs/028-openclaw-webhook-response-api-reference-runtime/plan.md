...
# 实施计划：028-openclaw-webhook-response-api-reference-runtime

## 目标

冻结 Watchdog 与 OpenClaw 之间的 webhook / response contract，并提供最小 reference runtime，证明宿主只做渠道适配。
...
| 层 | 文件落点 | 责任 |
|---|---|---|
| Contract Docs | `docs/architecture/openclaw-codex-watchdog-full-product-loop-design.md`, `docs/getting-started.zh-CN.md` | webhook / response contract 与宿主边界 |
| API Surface | `src/watchdog/api/openclaw_callbacks.py`, `src/watchdog/api/openclaw_responses.py` | 对 OpenClaw 暴露稳定接口 |
| Reference Runtime | `examples/openclaw_watchdog_client.py`, `examples/openclaw_webhook_runtime.py` | 最小宿主模板 |
| 验证 | `tests/integration/test_openclaw_integration_spine.py`, `tests/test_openclaw_contracts.py` | 契约、shape、reference runtime |
...
- OpenClaw webhook callback ingress/egress
- response API shape
