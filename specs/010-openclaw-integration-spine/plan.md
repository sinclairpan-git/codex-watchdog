...
# 实施计划：010-openclaw-integration-spine

## 目标

在不实现完整实时监管与自动恢复体系的前提下，交付一套 **可长期演进的 OpenClaw 集成脊柱**：通过中立 contract 包、L2 稳定投影层、L3 独立 adapter spine，以及 canonical `WatchdogAction -> WatchdogActionResult` 动作面，让 OpenClaw 只依赖稳定契约而不再耦合 raw Watchdog / A-Control-Agent 字段。
...
| L3 Adapter | `src/watchdog/services/adapters/openclaw/intents.py`, `reply_model.py`, `adapter.py` | OpenClaw intent/reply 映射 |
...
- `approve_approval` / `reject_approval` 封装现有 A-Control-Agent decision 路径
...
### Phase 5：接入 L3 OpenClaw adapter spine
...
- contract 包禁止使用 `openclaw` 命名
- adapter 单独放在 `services/adapters/openclaw/`
...
5. tasks 的文件落点与上述边界一致，而不是继续指向 `src/watchdog/services/openclaw/*`。
