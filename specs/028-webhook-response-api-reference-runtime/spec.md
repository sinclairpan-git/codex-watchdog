...
# Feishu Webhook / Response API / Reference Runtime — 功能规格说明

## 概述

`028-webhook-response-api-reference-runtime` 是完整产品闭环中的 `WI-5`。它的目标是把“Feishu 如何接入”从口头说明收成稳定契约和最小参考宿主实现。

## 功能需求

- **FR-2801**：028 必须定义 Watchdog -> Feishu 的主动回调入口：
  - `POST /feishu/v1/watchdog/envelopes`
- **FR-2802**：028 必须定义 Feishu -> Watchdog 的结构化响应入口：
...
- **FR-2804**：028 必须冻结 webhook 成功响应 contract，至少包含：
  ...
  - `receipt_id`
  - `received_at`
- **FR-2805**：028 必须冻结 Feishu 回流 Watchdog 的 response contract，至少包含：
  - `envelope_id`
  - `envelope_type`
  - `approval_id`
  - `decision_id`
...
- **FR-2807**：028 必须提供最小 Feishu reference runtime / template，证明宿主只消费稳定 envelope / response contract，而不拥有第二套业务内核。
  - 最小模板至少包含 `examples/codex_watchdog_client.py` 与 `examples/feishu_webhook_runtime.py`
- **FR-2808**：028 必须把宿主边界写清：
  - Feishu 只做接收、渲染、输入、回传
  - Feishu 不做决策、不做风险分类、不做第二套状态机
...
### 用户故事 1：Feishu 可以按固定契约接入，而不是继续靠口头说明
