...
在 `010-019` 已经建立 stable session spine、events、receipts、recovery、supervision、workspace activity 与 OpenClaw adapter 之后，当前仍有一个明显的 legacy write seam：

- A-Control-Agent 已经提供 `POST /api/v1/tasks/{project_id}/steer`；
- Watchdog 内部也已经通过 `continue_session` / `evaluate_supervision` 间接复用 steer；
- 但上层调用方还没有一个稳定、版本化、可幂等的“显式人工指导”动作模型，只能继续依赖 raw `/steer`。

020 的唯一目标固定为：

> 建立 stable operator-guidance write surface，让 OpenClaw 与其他上层调用方能够通过 Watchdog canonical `WatchdogAction` 提交最小可控的人类指导消息，而不是直接绑定 A-Control-Agent raw `/steer`。
...
- **FR-2005**：020 的动作执行必须复用既有 `post_steer(...)` executor，不得新开直连 A-Control-Agent 的平行实现。
...
- **FR-2008**：020 必须支持 OpenClaw adapter intent：
...
OpenClaw 需要向某个 session 注入一段最小人工指导，而不直接依赖 raw `/steer`。
...
同一个 operator guidance 请求在 HTTP canonical、HTTP alias 和 OpenClaw adapter 上必须复用同一套稳定执行结果。
...
已有脚本与历史调用可能仍直接依赖 A-Control-Agent raw steer route。
