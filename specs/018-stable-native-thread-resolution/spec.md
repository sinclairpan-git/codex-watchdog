...
- 但一旦调用方手里只有 `native_thread_id`，仍然只能回退到 A-Control-Agent raw `/api/v1/tasks/by-thread/{thread_id}`，而不是继续走 Watchdog stable contract。

017 的 session directory 虽然能靠全量枚举再扫描解决一部分问题，但它不是“给一个 native thread 直接解回 stable session”这一条明确契约。

018 的唯一目标固定为：

> 建立 stable native-thread resolution read surface，让 OpenClaw 与其他调用方能够在仅持有 `native_thread_id` 时，直接解析出同源的稳定 `SessionProjection` reply，而不是扫描 session directory 或直接绑定 A-Control-Agent raw `/by-thread` payload。

## 功能需求

- **FR-1801**：018 必须补一个独立 stable native-thread resolution read surface；不得把稳定语义直接塞回 A-Control-Agent raw `/api/v1/tasks/by-thread/{thread_id}`。
...
- **FR-1805**：native-thread resolution 必须先通过 A-Control-Agent 的 `/api/v1/tasks/by-thread/{thread_id}` 获得目标 task row，再在 Watchdog L2 内把该 row 投影为稳定 `SessionProjection`；adapter 与 HTTP route 均不得自己拼 raw payload。
...
- **FR-1808**：OpenClaw adapter 必须新增 `get_session_by_native_thread` intent，并通过 stable builder 返回与 HTTP route 同源的 `ReplyModel`；该 intent 不要求 `project_id`，但必须要求 `arguments.native_thread_id`。
...
- **FR-1810**：README、getting-started 与 OpenAPI 必须明确把 native-thread resolution 纳入 stable read surface，并说明：
  ...
  - `/api/v1/tasks/by-thread/{thread_id}`：A-Control-Agent raw lookup route，继续存在但不承担 stable contract 角色
- **FR-1811**：018 必须补齐 API 测试、adapter 测试、最小 integration 测试，以及 raw `/api/v1/tasks/by-thread/{thread_id}` 非回归测试。

### 用户故事 1：OpenClaw 手里只有 native thread，也能直接回到 stable session

OpenClaw 可能从事件流、审批投影或外部上下文中拿到 `native_thread_id`，但当下并不知道 `project_id`。
...
同一个 `native_thread_id` 在 HTTP stable route 与 OpenClaw adapter 上应该得到同源 session reply，而不是两套 lookup 逻辑。
...
已有脚本或测试可能仍依赖 A-Control-Agent raw `/api/v1/tasks/by-thread/{thread_id}`。
