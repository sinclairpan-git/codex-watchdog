...
# 实施计划：036-feishu-control-plane-and-feishu-retirement

## 目标

把现有人机交互主路径从 Feishu webhook/callback 迁到 Feishu 控制面，同时冻结 interaction context、通知投递中间态、actor binding、ACL 与 supersede 纪律，使 Feishu 只保留兼容读写协议。

## 架构摘要

- **Feishu is the only primary control plane**：`src/watchdog/services/feishu_control/service.py` 与 `src/watchdog/api/feishu_control.py` 承载主控制入口；高风险确认、人工 override、通知回执与窗口续发语义都从这里进入，而不是继续由 Feishu callback 主导。
...
- **Feishu becomes compatibility-only**：`feishu_bootstrap.py`、`feishu_callbacks.py` 与 `FeishuAdapter` 继续提供迁移期读写协议，但不能再决定主链路、交互有效性或通知真相。
- **Main wiring makes the boundary visible**：`src/watchdog/main.py` 必须显式注册 Feishu 主入口，并让 Feishu surface 的文案和依赖关系都体现“兼容层”而不是“主控制面”。
...
| Main app wiring | `src/watchdog/main.py` | 注册 Feishu 主控制面 API，初始化主路径依赖，并把 Feishu surface 降级为 compatibility-only |
...
| Event schema and read-side projection | `src/watchdog/services/session_service/models.py`, `src/watchdog/services/session_spine/projection.py` | 冻结 `interaction_context_superseded`、`interaction_window_expired` 等事件类型及其读侧投影，确保 Feishu/Feishu 共用同一套事件语义 |
| Compatibility surfaces | `src/watchdog/api/feishu_bootstrap.py`, `src/watchdog/api/feishu_callbacks.py`, `src/watchdog/services/adapters/feishu/adapter.py` | 保留迁移期 bootstrap / callbacks / stable read-write compatibility contract，但统一复用 control-plane interaction validity 与 write barrier discipline |
| Delivery and interaction semantics | `tests/test_watchdog_notification_delivery.py`, `tests/test_watchdog_feishu_control.py`, `tests/test_feishu_contracts.py` | 冻结通知投递中间态、单活跃 interaction family、旧上下文审计化、Feishu DM 限制与 Feishu compatibility 边界 |

## 边界纪律

- 036 不得让任何 Feishu 或 Feishu handler 直接修改 approval/delivery/runtime store 而不先写 `Session Service` event。
- 036 不得把 Feishu bootstrap/callbacks 换个名字继续当主控制面；它们只能消费共享 control-plane contract。
...
1. **先冻结 formal docs 与 036 work item 元数据**
   ...
   - 明确当前仓库的主控制入口仍偏向 Feishu compatibility surface，需要在 036 中调转主次。
2. **再写失败测试锁定 Feishu control semantics**
   - 先证明当前仓库缺少 Feishu 主控制服务、ACL 与 interaction validity contract；
   - 先证明当前通知投递与 Feishu callbacks 还没有单活跃 context / supersede / stale reply 的正式锁定。
...
4. **再收口 Feishu compatibility 与 main wiring**
   - 让 `main.py` 以 Feishu surface 为主入口；
   - 让 Feishu bootstrap/callbacks/adapter 只保留迁移期兼容协议与审计入口。
5. **最后运行验证并更新 handoff**
   - 固定后续 e2e work item 只消费 Feishu control contract 与 notification truth，不再从 Feishu 入口补主链路语义。
...
- `specs/036-feishu-control-plane-primary-surface/spec.md`
- `specs/036-feishu-control-plane-primary-surface/plan.md`
- `specs/036-feishu-control-plane-primary-surface/tasks.md`
- `specs/036-feishu-control-plane-primary-surface/task-execution-log.md`
- `.ai-sdlc/work-items/036-feishu-control-plane-primary-surface/*`

关键原则：

- 先把 036 收紧为“Feishu 主控制面 + Feishu compatibility-only”；
...
### Phase 4：收口 Feishu compatibility、通知投递恢复与主入口切换

交付内容：

- `src/watchdog/api/feishu_bootstrap.py`
- `src/watchdog/api/feishu_callbacks.py`
...
- Feishu 只保留稳定读写兼容协议与迁移期 bootstrap，不再主导控制真相；
...
- `specs/036-feishu-control-plane-primary-surface/task-execution-log.md`
- `.ai-sdlc/work-items/036-feishu-control-plane-primary-surface/latest-summary.md`

关键原则：

- handoff 必须明确 Feishu 是唯一主控制面，Feishu 只剩 compatibility-only surface；
- Task 8 只能消费 036 冻结的 interaction/notification/control-plane contract，不再补回 Feishu 主链路语义。
...
- `tests/test_feishu_contracts.py`
  - Feishu 入口退为 compatibility-only surface
  - bootstrap/callbacks 复用共享 control-plane contract，不再独立决定有效性
  - Feishu 仍可提供 stable read / action receipt / migration callback，但不能直接成为主控制面
- `tests/test_watchdog_ops.py`
...
### 风险 3：Feishu 名义上退役，实际上仍承担主职责

应对：

- `main.py` 显式注册 Feishu 为主入口；
- Feishu tests 只验证 compatibility contract，不再验证主链路决策。
...
5. `main.py`、Feishu bootstrap/callbacks/adapter 已体现 compatibility-only 边界，而不是继续承担主控制职责；
6. 后续 e2e work item 可以直接消费 036 冻结的 Feishu/control-plane contract，而不需要补回 Feishu 主链路语义。
