# 任务分解：036-feishu-control-plane-and-feishu-retirement

> 对应规格：`specs/036-feishu-control-plane-primary-surface/spec.md`
>
> 对应计划：`specs/036-feishu-control-plane-primary-surface/plan.md`

## Task 36.1 冻结 036 formal docs 与 Feishu / Feishu 执行边界

- **任务编号**：T361
- **状态**：已完成（2026-04-14）
- **目标**：把总实施计划中的 `Task 7` 正式拆成 canonical execution work item，并把 036 收紧为 Feishu 主控制面、interaction validity 与 Feishu compatibility-only 边界。
- **文件**：
...
- **可并行**：否
- **验收标准**：
  1. 已明确 036 的主目标是 Feishu 成为唯一主控制面，Feishu 退为 compatibility-only surface；
...
## Task 36.2 写失败测试锁定 Feishu control、通知投递中间态与 Feishu compatibility contract

- **任务编号**：T362
- **状态**：已完成（2026-04-14）
- **目标**：用失败测试先锁定 Task 7 的正式输入输出 contract，避免实现继续把 Feishu callback 当主控制入口。
- **文件**：
...
- **验收标准**：
  ...
  6. 覆盖 Feishu 入口退为 compatibility-only surface，而非主执行入口。
- **验证**：
  - `uv run pytest -q tests/test_watchdog_feishu_control.py tests/test_watchdog_notification_delivery.py tests/test_feishu_contracts.py`
- **完成情况**：
  ...
  3. 已在 `tests/test_feishu_contracts.py` 增补主 Feishu route 与 Feishu compatibility route 并存的 contract。
...
## Task 36.4 收口通知投递恢复与 Feishu compatibility 边界

- **任务编号**：T364
- **状态**：已完成（2026-04-14）
- **目标**：把通知投递中间态与 Feishu 兼容层真正接入主路径，使旧入口不再承担主控制职责。
- **文件**：
...
- **验收标准**：
  ...
  4. Feishu bootstrap/callbacks 只保留迁移期兼容协议，并复用共享 control-plane validity contract；
  5. ops/read-side 能看到 Feishu 主控制面与通知中间态事实，且不回退。
- **验证**：
  - `uv run pytest -q tests/test_watchdog_notification_delivery.py tests/test_feishu_contracts.py tests/test_watchdog_ops.py`
- **当前进展**：
  1. 已把 interaction metadata 贯通到 `delivery/envelopes.py`、`delivery/worker.py` 与 `session_spine/projection.py`；
  2. 已锁定 Feishu 主 route 与 Feishu compatibility route 同时存在的 contract；
  3. `feishu_bootstrap.py` 触发 legacy requeue 时，已开始镜像 `interaction_context_id` / `interaction_family_id` / `actor_id` 等 metadata 到 canonical `notification_requeued` 事件；
  4. `feishu_callbacks.py` 与 `FeishuAdapter` 已显式标注 compatibility-only 语义；
  5. `feishu_responses` 现在也先写 compatibility receipt，再进入 approval / human override side effects，旧入口已遵守同一条 event-first 纪律。
...
- **文件**：
  - `specs/036-feishu-control-plane-primary-surface/task-execution-log.md`
  - `.ai-sdlc/work-items/036-feishu-control-plane-primary-surface/execution-plan.yaml`
  - `.ai-sdlc/work-items/036-feishu-control-plane-primary-surface/runtime.yaml`
  - `.ai-sdlc/work-items/036-feishu-control-plane-primary-surface/resume-pack.yaml`
...
- **验收标准**：
  ...
  4. handoff 明确 Feishu 只保留 compatibility-only surface，不得回升为主控制面。
- **验证**：
...
- **完成情况**：
  ...
  2. 已记录 docs baseline、Feishu primary control surface、Feishu compatibility bootstrap/receipt 三个连续提交；
  3. 已记录实现期对抗 reviewer 多次超时未返回 blocking/P1 结论，当前以 79 个相关回归作为本轮收口证据。

## 整体验收

- 036 是长时运行自治一期的 `Task 7`，只负责把 Feishu 收敛为唯一主控制面，并把 Feishu 降为 compatibility-only surface。
- 036 完成后，审批确认、通知回执、人工 override 与交互窗口更新应统一走 event-first write barrier。
- 036 完成后，同一 interaction family 只能有一个活跃上下文；旧上下文晚到消息只进审计。
- 036 已完成 Feishu 主控制面迁移与 Feishu 主链路退役的一期 contract 收口；端到端 golden path 与一期通关验收留给后续 work item。
