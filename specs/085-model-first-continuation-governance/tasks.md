...
- [x] T857 实现 `ContinuationPacket` 真值对象与 packet->markdown/adapter summary 的严格单向渲染契约，并把 A-side handoff markdown、Watchdog recovery/resume、adapter summary 全部切到 packet truth，禁止渲染结果回流为 `latest-summary`、`PCDI`、routing input、dedupe 输入或模型输入。
