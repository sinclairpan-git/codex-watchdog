...
# M5 加固与 PRD 交付缺口（非飞书 / 非 OpenClaw 渠道）— 功能规格说明

## 概述

在 **不实现飞书机器人、不实现 OpenClaw 渠道插件** 的前提下（PRD §3.2），补齐 PRD **§18 M5** 与 **§14.3 可观测性**、**§19.1 对 Codex app-server 的对接准备**、**§22 部分交付物** 中仍可由本仓库独立完成的部分。

## 功能需求

- **FR-401**：A-Control-Agent 与 Watchdog 均暴露 **`GET /metrics`**（Prometheus 文本格式），至少包含：任务数、审计中可聚合的 handoff / resume / steer / 审批相关事件计数（与 PRD §14.3「任务数、卡顿次数、自动 steer 次数、审批数、恢复次数」对齐；其中「卡顿」以 Watchdog 侧 `steer_injected` 等可观测动作为代理，若审计无对应 action 则记 0）。
...
- **FR-406**：交付 **OpenAPI JSON 导出脚本** 与 **OpenClaw 侧调用 Watchdog 的示例脚本**（仅 HTTP，无飞书），满足 PRD §22 中「API 文档」「集成适配脚本」的最低可交付形态。
...
- 不实现飞书、不实现 OpenClaw 插件、不要求真实 Codex app-server 在线。
