...
在 `023` 完成真实 Codex 客户端接缝与 OpenClaw 路由模板之后，仓库已经具备：

- A 侧真实事实读取与最小动作执行能力；
- B 侧 stable session spine / facts / explanations / action surfaces；
- OpenClaw 可直接消费的稳定路由模板。
...
- **FR-2401**：024 必须在 `Watchdog(B)` 内新增常驻 `Projection Worker`，持续从 `A-Control-Agent` 拉取或接收事实变化，而不是只在应用启动时执行一次监督逻辑。
...
- **FR-2406**：024 必须为查询层补齐 freshness 语义，至少允许 read bundle 判断：
  ...
  - 当前快照是否为上一次持久化结果
  - 上次成功刷新时间
- **FR-2407**：024 完成后，stable session 查询面必须优先读取 canonical 持久化 session spine，而不是继续在 read API 内直接向 A 侧临时拼装核心 session 视图。
...
- **FR-2412**：024 不得实现 OpenClaw webhook、Feishu 渠道 runtime 或用户响应回流；这些都留给后续 work items。
