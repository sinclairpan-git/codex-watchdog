...
在 `010-openclaw-integration-spine` 冻结 stable read/write spine、`011-stable-session-events` 冻结只读 stable event surface、`012-stable-recovery-execution` 冻结真实恢复执行动作之后，`013-stable-action-receipts` 的目标是继续补齐 **稳定 receipt 查询面**：让 OpenClaw 与其他上层调用方能够读取 Watchdog 已持久化的幂等动作结果，而不是通过重复提交动作、猜测副作用是否已发生，或绑定 legacy / transport 层返回体。
...
- **FR-1301**：013 必须在现有中立 `session_spine` contract 上新增稳定收据查询语义；不得把 receipt 查询塞进 `openclaw` 命名空间或 legacy 路由中。
...
- **FR-1308**：OpenClaw adapter spine 必须新增 `get_action_receipt` intent，并将其映射到 stable `ReplyModel`；adapter 不得直接读取 `action_receipts.json`。
- **FR-1309**：013 的 receipt 查询必须只依赖 Watchdog 本地 `ActionReceiptStore`；不得重新访问 A-Control-Agent、不得重放 side effect、不得调用 legacy recover / approvals / steer 路由来推断结果。
...
OpenClaw 在提交 `continue_session`、`approve_approval`、`execute_recovery` 之后，需要能按同一幂等键读取已持久化结果，而不是再次执行相同动作。
...
场景 2：上述未找到结果不触发任何 side effect，也不访问 A-Control-Agent。
...
- 不让 OpenClaw adapter 直接读本地 receipt 存储文件。
