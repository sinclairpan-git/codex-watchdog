...
# 实施计划：Feishu × Codex 双机会话监管系统

## 目标与范围

交付 Codex runtime service（runtime 机器）、Watchdog / Policy Engine（watchdog 机器）、A/B HTTPS+Token 通信、统一数据模型与审计，以及 Feishu 侧调用 Watchdog 的集成方式。不包含飞书机器人与 Feishu 渠道插件本身（PRD §3.2）。
...
- B 仅通过 Codex runtime service 业务 API 监管，不直连 Codex 原始协议、不控 runtime 机器桌面（PRD §4）。
