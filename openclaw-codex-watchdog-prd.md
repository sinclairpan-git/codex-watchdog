# OpenClaw × Codex 双机会话监管系统开发需求文档（PRD + Technical Spec）

- 文档名称：OpenClaw × Codex 双机会话监管系统
- 版本：V1.0
- 日期：2026-04-03
- 状态：可直接进入 Codex / Cursor 开发
- 适用范围：
  - A 机：运行 Codex、Codex app-server、被开发仓库、A-Control-Agent
  - B 机：运行 OpenClaw、Watchdog/Policy Engine
  - 消息入口：飞书
- 已知前提：
  - OpenClaw ↔ 飞书通路已存在
  - 本项目不负责飞书机器人接入与 OpenClaw 渠道打通
  - 本项目从 OpenClaw 已接收到飞书消息之后开始
...
用户当前使用方式为：
- A 机使用 Codex 执行长期开发任务
- B 机运行 OpenClaw，作为统一的外部交互与监管入口
- 已经具备 OpenClaw ↔ 飞书消息通路

当前痛点：
1. 无法在飞书中随时询问 A 机当前开发任务进展
2. Codex 长任务在出现卡顿、等待下一步、上下文压力过高、循环失败时，缺少自动处理机制
3. 当需要权限审批时，无法按风险等级自动处理或精准提醒人工处理
4. 缺少 A/B 双机之间的标准化通信协议与可恢复的任务线程管理能力

因此需要建设一套“双机会话监管系统”，让 B 机上的 OpenClaw 能够通过结构化接口监管 A 机上的 Codex 开发会话，并在飞书中提供问答、自动处理、授权分级和异常恢复能力。
...
本项目包括：
1. A 机的 A-Control-Agent 服务开发
2. B 机的 Watchdog / Policy Engine 服务开发
3. A/B 间的内网通信协议与鉴权机制
4. 项目任务、线程、审批、状态、日志的数据模型
5. OpenClaw 调用 Watchdog 的集成接口
...
本项目不包括：
1. 飞书机器人、OpenClaw 渠道插件本身的安装与接入
2. A 机桌面 UI 层级的视觉监控或自动点击
3. 完全无人值守地自动批准所有高风险命令
4. git push / release / 发布流程自动化
5. 远程桌面控制、通用 SSH 运维入口
6. 对 OpenClaw 核心代码进行深度侵入式修改（优先通过外置服务集成）
...
1. A 负责执行，B 负责监管
2. B 不直接控制 A 的桌面 UI
3. B 不直接调用 Codex 原始底层协议，而通过 A-Control-Agent 获取业务化接口
...
- 作为 Codex 协议层入口
- 提供线程、历史、审批、事件流能力
- 为 A-Control-Agent 提供底层会话接口

##### 4.3.2 A-Control-Agent
职责：
- 对上提供标准化任务接口
...
##### 4.3.3 Repo Watcher（可内嵌到 A-Control-Agent）
职责：
- 监听文件修改
- 统计最近一段时间是否存在有效变更
- 辅助判断“真卡顿”与“长测试执行中”
...
期望：
- OpenClaw 返回当前任务阶段、最近摘要、最近改动文件、阻塞点、审批状态、上下文压力
...
OpenClaw ↔ 飞书链路已存在，因此本项目不实现飞书通道接入。

### 11.2 OpenClaw 侧需要完成的集成
...
1. A-Control-Agent 不得直接暴露公网
2. A/B 之间推荐通过 Tailscale / 内网 VPN 通信
3. A-Control-Agent 默认仅监听本机或私网地址
4. 所有 A/B 调用必须带 token 鉴权

### 13.2 权限边界

1. B 不得获取 A 的通用 shell 远程执行能力
2. A-Control-Agent 仅提供白名单业务接口
3. 高风险命令必须人工审批
...
1. A-Control-Agent 异常重启后，能从持久化数据恢复 project_id ↔ thread_id 映射
2. B 侧 Watchdog 异常重启后，不丢失待审批与最近动作状态
...
6. 在 A-Control-Agent 重启后，原 project_id 任务仍可查询状态

### 17.2 安全验收

1. A-Control-Agent 未暴露公网
2. 所有调用均有 token 校验
3. L3 命令无法自动通过
4. 所有自动动作均有审计日志
...
- A-Control-Agent 启动
- 创建任务 / 查询状态 API 打通
- OpenClaw → Watchdog → A-Control-Agent 查询链路打通
...
按以下顺序实现：
1. 先实现 A-Control-Agent 的任务状态持久化
2. 再接 Codex app-server 的线程与事件能力
3. 再实现 Watchdog 的状态轮询与规则判断
4. 最后接 OpenClaw 的查询/控制/审批路由
...
2. 不得把 A-Control-Agent 设计成任意命令执行器
3. 不得默认自动通过高风险命令
...
2. OpenClaw 与飞书通路已存在，本项目只需消费其上游消息
...
1. A-Control-Agent 服务代码
2. Watchdog / Policy Engine 服务代码
3. OpenClaw 集成适配代码或脚本
4. 配置样例（A/B 两侧）
