...
这组检查默认统一验证 `GET /healthz`、飞书 `url_verification`、OpenAI-compatible provider 接线与失败回退、以及 Memory Hub preview route。`--target feishu-control` 额外补一条 repo-local 的 Feishu callback contract smoke：它会向 `/api/v1/watchdog/feishu/events` 发送 `im.message.receive_v1` 的 DM 文本事件，内容为 `repo:<project_id> /goal <goal_message>`，借此确认官方事件入口能被归一成 `goal_contract_bootstrap`。`--target feishu-discovery` 则单独验证多项目目录发现链路：它会发送一条默认文本为 `项目列表` 的 Feishu DM synthetic event，要求返回 `list_sessions/session_directory`，并覆盖 `WATCHDOG_SMOKE_FEISHU_DISCOVERY_EXPECTED_PROJECT_IDS` 中声明的项目集合。`--target provider` 默认仍走 synthetic contract；只有显式设置 `WATCHDOG_SMOKE_PROVIDER_LIVE=true` 时，success probe 才会真实请求当前配置的外部 provider，failure probe 仍会验证 timeout 降级路径。该检查需要先配置 `WATCHDOG_SMOKE_FEISHU_CONTROL_PROJECT_ID`、`WATCHDOG_SMOKE_FEISHU_CONTROL_GOAL_MESSAGE`，以及用于多项目发现验收的 `WATCHDOG_SMOKE_FEISHU_DISCOVERY_EXPECTED_PROJECT_IDS`；若未配置会返回 `skipped`，若已配置但契约不一致则直接失败。若真实数据量较大导致 callback contract 超过默认 3 秒，可分别设置 `WATCHDOG_SMOKE_FEISHU_CONTROL_HTTP_TIMEOUT_S` 与 `WATCHDOG_SMOKE_FEISHU_DISCOVERY_HTTP_TIMEOUT_S` 放宽对应烟测窗口，而不影响其余 target；若真实 provider 响应更慢，可单独设置 `WATCHDOG_SMOKE_PROVIDER_HTTP_TIMEOUT_S`。若需要沉淀验收材料，可追加 `--markdown-report <path>` 生成脱敏后的 Markdown 记录。
