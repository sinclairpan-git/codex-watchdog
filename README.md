...
这组检查默认统一验证 `GET /healthz`、飞书 `url_verification`、OpenAI-compatible provider 接线与失败回退、以及 Memory Hub preview route。`--target feishu-control` 额外补一条 repo-local 的 Feishu callback contract smoke：它会向 `/api/v1/watchdog/feishu/events` 发送 `im.message.receive_v1` 的 DM 文本事件，内容为 `repo:<project_id> /goal <goal_message>`，借此确认官方事件入口能被归一成 `goal_contract_bootstrap`。这条 smoke 会进入真实控制写路径，所以 `WATCHDOG_SMOKE_FEISHU_CONTROL_PROJECT_ID` 与 `WATCHDOG_SMOKE_FEISHU_CONTROL_GOAL_MESSAGE` 必须对应一个经操作者确认的可写 live target；缺失时结果会以 `operator_confirmation_required` 的 `skipped` 明确提示，并在 evidence 中要求 `confirm_mutating_live_target`，而不是替你猜值继续执行。没有经确认的 `project_id + goal_message` 组合时，整包验收必须继续按 fail-closed 处理，不能把 `skipped` 当通过。`--target feishu-discovery` 则单独验证多项目目录发现链路：它会发送一条默认文本为 `项目列表` 的 Feishu DM synthetic event，要求返回 `list_sessions/session_directory`，并覆盖 `WATCHDOG_SMOKE_FEISHU_DISCOVERY_EXPECTED_PROJECT_IDS` 中声明的项目集合；如果 structured `progresses[]` 中存在 `recovery_outcome=new_child_session`，还要求 message 里的子会话展示与 `recovery_child_session_id` 归一后一致。`--target provider` 默认仍走 synthetic contract；只有显式设置 `WATCHDOG_SMOKE_PROVIDER_LIVE=true` 时，success probe 才会真实请求当前配置的外部 provider，failure probe 仍会验证 timeout 降级路径。该检查需要先配置上述 `feishu-control` live target，以及用于多项目发现验收的 `WATCHDOG_SMOKE_FEISHU_DISCOVERY_EXPECTED_PROJECT_IDS`；若 discovery 未配置会返回 `skipped`，若已配置但契约不一致则直接失败。若真实数据量较大导致 callback contract 超过默认窗口，可分别设置 `WATCHDOG_SMOKE_FEISHU_CONTROL_HTTP_TIMEOUT_S` 与 `WATCHDOG_SMOKE_FEISHU_DISCOVERY_HTTP_TIMEOUT_S` 放宽对应烟测窗口；其中 discovery smoke 默认使用 30 秒窗口，而不影响其余 target。若真实 provider 响应更慢，可单独设置 `WATCHDOG_SMOKE_PROVIDER_HTTP_TIMEOUT_S`。若需要沉淀验收材料，可追加 `--markdown-report <path>` 生成脱敏后的 Markdown 记录。
...
- `GET /api/v1/watchdog/sessions/{project_id}/event-snapshot` 返回稳定 `ReplyModel(reply_code=session_event_snapshot, events=SessionEvent[])` JSON snapshot；缺失 raw `event_id` 时会先补 deterministic synthetic id，再按 repeated `event_id` 去重
...
它与 011 的 stable SSE route 共享同一份 event schema version；snapshot 读取会把
projected raw 任务事件与 selected canonical `Session Service` 事件合并，从而把
recovery / goal-contract lineage 一并外露；缺失 raw `event_id` 时会先补 deterministic
synthetic id，再按 repeated `event_id` 去重，并在 control link 降级时回退到这些
canonical event。它不新增 follow / cursor / backfill 语义；`follow=true` 现在会先发
一段 stable bootstrap snapshot：其中包含 projected raw snapshot 事件，以及 continuity
相关的 selected canonical `Session Service` 事件；随后再进入 projected raw SSE 实时流，
并按 `event_id` 去重 A 侧可能重放的 backlog；如果 raw 事件缺失 `event_id`，则先按稳定
字段指纹生成 deterministic synthetic id 后再去重。若 stream startup 失败，则回退到已拿到的
bootstrap event。
OpenClaw adapter 同步新增 `list_session_events` intent，继续只消费稳定
`SessionEvent[]`，不直读 raw SSE。
